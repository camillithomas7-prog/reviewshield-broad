...
.wa-form input{padding:7px 8px;border:1px solid #e8e8e8;border-radius:6px;background:#fafafa;color:#333;font-size:11px;font-family:inherit;flex:1;min-width:70px}
...
<div class="container">
  ...
  <div class="search-bar">
    ...
    <input type="text" id="local-search" placeholder="Cerca per nome, cognome, telefono, email..." oninput="localSearch(this.value)">
  </div>
  ...
  <div class="reminder-bar active">
    <div class="reminder-bar__title">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2" stroke-linecap="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>
...
<?php if ($tab === 'team'): ?>
  <h2 style="font-size:16px;color:#111;margin-bottom:4px">Team & Operatori</h2>
  <p style="font-size:12px;color:#888;margin-bottom:14px">Aggiungi operatori per assegnare i lead</p>
  <div class="team-grid">
...
<?php else: ?>
  ...
  <div class="wa-form">
    <div class="title">Aggiungi Lead da WhatsApp / manuale</div>
    <form method="POST">
  ...
  <div class="lead <?=$is_scaduto?'scaduto':''?>" id="lead-<?=$c['id']?>" data-search="<?=htmlspecialchars($search_blob)?>" style="border-left-color:<?=$sc?>">
      ...
      <div style="flex:1;min-width:0">
        <div id="nview-<?=$c['id']?>">
          <div class="lead__name">
            <?php if($is_wa): ?><span title="Lead WhatsApp" style="display:inline-block;width:10px;height:10px;background:#25d366;border-radius:50%"></span><?php endif; ?>
    ...
    <div class="lead__actions">
      ...
      <a href="<?=wa_link($c['telefono']??'')?>" target="_blank" class="btn-wa">WA</a>
      <a href="<?=wa_link($c['telefono']??'', $reminder_msg)?>" target="_blank" class="btn-wa-r" title="Invia messaggio di reminder">Reminder</a>
